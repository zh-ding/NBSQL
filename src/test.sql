...
select student.name from student left outer join scores on student.ID=scores.student_id join course on scores.course_id=course.ID;
...
--select student.name, course.id, teacher.name, scores.score from student join scores on student.ID=scores.student_id join course on scores.course_id=course.ID right outer join teacher on course.teacher_id=teacher.ID where (teacher.ID=16 OR scores.score>=95) AND (teacher.name='tuixiu' OR scores.score <= 100);
drop table course;
drop table teacher;
drop table student;
drop table scores;
